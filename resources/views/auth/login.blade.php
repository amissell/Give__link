...
<div class="relative w-full max-w-md bg-white p-6 rounded-xl shadow-lg border border-gray-200 overflow-hidden">
    ...
    <div class="relative z-10">
        <h2 class="text-xl font-semibold text-gray-800 mb-4 text-center animate-float">Login</h2>
        ...
        <form action="{{ route('login') }}" method="POST" class="reveal-item">
            @csrf
            <div class="mb-4">
                <label for="email" class="block text-gray-700">Email</label>
                <input type="email" id="email" name="email" class="w-full px-3 py-2 border rounded-md focus:ring-2 focus:ring-emerald-500 transition-all" value="{{ old('email') }}" required>
            </div>
            <div class="mb-4">
            ...
            <button type="submit" class="w-full px-4 py-2 bg-emerald-600 text-white rounded-md hover:bg-emerald-700 transition-all transform hover:scale-[1.02] focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:ring-offset-2">Login</button>
        </form>

        <!-- Forgot Password Link -->
        <div class="mt-4 text-center reveal-item delay-100">
            <a href="{{ route('password.request') }}" class="text-sm text-emerald-600 hover:text-emerald-700 relative group">
                Forgot your password?
                <span class="absolute bottom-0 left-0 w-0 h-0.5 bg-emerald-600 transition-all duration-300 group-hover:w-full"></span>
            </a>
        </div>
        
        <div class="mt-2 text-center reveal-item delay-200">
            <span class="text-sm text-gray-600">Don't have an account?</span>
            <a href="{{ route('register') }}" class="text-sm text-emerald-600 hover:text-emerald-700 font-medium relative group ml-1">
                Register
                <span class="absolute bottom-0 left-0 w-0 h-0.5 bg-emerald-600 transition-all duration-300 group-hover:w-full"></span>
            </a>
        </div>
